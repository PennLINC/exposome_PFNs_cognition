...
# Separate out cognition at time 1 and time 2
nihtb_T1 <- nihtb[nihtb$eventname=="baseline_year_1_arm_1",c("subjectkey","nihtbx_picvocab_uncorrected","nihtbx_flanker_uncorrected","nihtbx_pattern_uncorrected","nihtbx_reading_uncorrected","nihtbx_picture_uncorrected")]
colnames(nihtb_T1)<-c("subjectkey","PicVocabT1","FlankerT1","PatternT1","ReadingT1","PictureT1")
...
colnames(nihtb_T2)<-c("subjectkey","PicVocabT2","FlankerT2","PatternT2","ReadingT2","PictureT2")
cog.data <- merge(nihtb_T1,nihtb_T2,by="subjectkey")
all.data.almost <- merge(pfn_sizes,cog.data[,c("subjectkey", setdiff(colnames(cog.data),colnames(pfn_sizes)))],by="subjectkey")
...
abcd.data.for.ridge<-abcd.data.traintest[,c("subjectkey","PicVocabT1","FlankerT1","PictureT1","PatternT1","ReadingT1","PicVocabT2","FlankerT2","PictureT2","PatternT2","ReadingT2","matched_group","interview_age","sex","meanFD","abcd_site","rel_family_id","ExpFactor")]
T2CogPred.for.ridge.complete<-abcd.data.for.ridge[complete.cases(abcd.data.for.ridge),]
abcd.data.for.ridge.complete$subjectkey <- gsub(pattern="NDAR_",replacement="",abcd.data.for.ridge.complete$subjectkey)

exp.data.T2CogPred.for.ridge<-T2CogPred.for.ridge.complete[,c("subjectkey","ExpFactor")]

write.csv(T2CogPred.for.ridge.complete,"/Users/askeller/Documents/ExposomeAnalyses/CodeForGithub/Step2_RidgeRegressions/ExpFactorPFNs-T2CogPred/data_for_ridge_T2CogPred.csv")
write.csv(exp.data.T2CogPred.for.ridge,"/Users/askeller/Documents/ExposomeAnalyses/CodeForGithub/Step2_RidgeRegressions/ExpFactorPFNs-T2CogPred/ExpFactor_features_T2CogPred.csv")
