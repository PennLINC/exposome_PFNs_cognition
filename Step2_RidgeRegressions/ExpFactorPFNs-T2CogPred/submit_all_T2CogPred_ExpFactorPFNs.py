...
tmpdir = '/cbica/projects/abcdfnets/scripts/keller_ridge/pennlinc-regression/example_keller/tempfiles_T2CogPred_ExpFactorPFNs/' #os.environ['TMPDIR']
networks = np.array(['all']) 
network = 'all'

GB = '200G'    

for cog_score in ['PicVocabT2','FlankerT2','PictureT2','PatternT2','ReadingT2']:
        os.system('qsub -l h_vmem={0},s_vmem={0} -N {1} -R y -V -j y -b y -o ~/sge/ -e ~/sge/ python {2}/T2CogPred_proc_predict_ExpFactorPFNs.py {2} {3} {4} {5}'.format(GB,cog_score,homedir,tmpdir,network,cog_score))
