...
control =['sex','meanFD','abcd_site','interview_age','PicVocabT1']
...
"""
phenotypes = pd.read_csv('/gpfs/fs001/cbica/projects/abcdfnets/data_for_ridge_T2CogPred.csv') #load
...
features = []
SESfeatures = pd.read_csv('/gpfs/fs001/cbica/projects/abcdfnets/ExpFactor_features_T2CogPred.csv') #load
SESfeaturesOrdered=SESfeatures[SESfeatures.subjectkey==phenotypes.subjectkey.values]['ExpFactor'] # re-order the participant IDs to match across dataframes
...
phenotypes[['PicVocabT2','rel_family_id','matched_group']].to_csv('/{0}/phenotypes_target.csv'.format(outdir))
