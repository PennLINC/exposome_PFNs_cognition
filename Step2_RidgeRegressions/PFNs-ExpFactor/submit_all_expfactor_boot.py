...
tmpdir = '/cbica/projects/abcdfnets/scripts/keller_ridge/pennlinc-regression/example_keller/tempfiles_PFNs_ExpFactor_boot/' #os.environ['TMPDIR']
...
for cog_score in ['ExpFactor']:
        os.system('qsub -l h_vmem={0},s_vmem={0} -N {1} -R y -V -j y -b y -o ~/sge/ -e ~/sge/ python {2}/expfactor_proc_predict_boot.py {2} {3} {4} {5}'.format(GB,cog_score,homedir,tmpdir,network,cog_score))
